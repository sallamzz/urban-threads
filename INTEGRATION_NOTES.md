...
|-------------|----------|------|
| Customer registration | `POST /integrations/customers` | APIKey |
| Custom events | `POST /integrations/events` | APIKey |
| Points balance | `GET /integrations/customers/{id}/points-balance` | APIKey |
| Tier progress | `GET /integrations/customers/{id}/tier-progress` | APIKey |
| Badges / campaigns | `GET /integrations/customers/{id}/campaigns` | APIKey |
...
All transactional calls (orders, hold, events) are proxied through Next.js Route Handlers — the `SecretKey` is never sent to the browser.
...
2. **Events** — Two events are tracked:
   - `profile_completed: {}` — fires once immediately after successful registration.
...
3. **Order Earn + Redeem** — At checkout:
   ...
   - `POST /integrations/orders` is called with `totalPaid` (post-discount amount — what Gameball uses for reward calculation), `lineItems` (with `category` as an array), and `channel: "web"`.
   - Hold is blocking: the flow aborts if the hold fails, preventing order submission without confirmed redemption.

4. **Customer Profile Page** — `/account` combines three API calls: points balance, tier progress, and campaigns (badges). Badges are rendered as achieved (full color) or unachieved (grayscale) based on `isAchieved`, `achievedCount`, or `completionPercentage >= 100`.

## Assumptions

- **100 pts = $1** for display purposes (matching the dashboard's `1pt = $0.01` redemption rate).
- Points shown in the UI are **available** (redeemable) points only — the API returns only the redeemable balance, excluding the 14-day pending window.
- No mobile number collected — `customerAttributes` uses name and email only.
- Orders are persisted in `localStorage` (no backend database). Gameball is the source of truth for points and tiers.
...
- **Error recovery**: Implement proper rollback — if the order API fails after a successful hold, call `DELETE /hold` to release the points immediately rather than waiting for automatic expiry.
